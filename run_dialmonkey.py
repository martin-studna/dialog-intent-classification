...
def main(args):
    ...
    formatter = DialMonkeyFormatter(
        path_prefix=os.path.join(os.path.abspath(
            os.path.curdir), 'dialmonkey/'),
    ...
    handler = ConversationHandler(
        conf, logger, should_continue=run_for_n_iterations(args.num_dials))
    handler.main_loop()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Run the Dialmonkey system with a specified configuration file')
