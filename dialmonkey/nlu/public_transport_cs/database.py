...
# form.
DATA_DIR = "../../../data/public_transport_cs"
...
def spell_number(num):
    ...
    if tens == 1:
        ...
        if units:
            ...
            spellings += ["{t}{u}".format(t=tens_str, u=units_str) for units_str in units_strs for tens_str in tens_strs]
            spellings += ["{u}{a}{t} ".format(t=tens_str, u=units_str, a='' if units == 1 else 'a')
...
def add_from_file(category_label, fname):
    ...
    with open(os.path.join(dirname, DATA_DIR, fname), encoding='utf-8') as stops_file:
